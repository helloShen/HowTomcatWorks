 ...
 */
import com.ciaoshen.howtomcatworks.ex03.connector.RequestStream;
...
public class HttpRequest implements HttpServletRequest {
   ...
   */
  /**
   * HttpProcessor解析完的queryString像下面这个样子，还没有解析完:
   *    "userName=tarzan&password=pwd"
   *
   * 这个parseParameters()函数调用
   * org.apache.Catalina.util.RequestUtil类的parseParameters()方法完成
   * 还是不需要自己造轮子
   * 解析完的结果储存在ParameterMap里
   */
  protected void parseParameters() {
    ...
    /**
     * 如果请求用的是POST方法，请求体里也会包含参数。
     * 下面这部分代码用来解析HTTP消息正文部分的参数。
     * 我们的例子里用的是GET方法，消息正文没有参数
     */
