...
import com.ciaoshen.howtomcatworks.ex03.connector.ResponseStream;
import com.ciaoshen.howtomcatworks.ex03.connector.ResponseWriter;
...
public class HttpResponse implements HttpServletResponse {
  ...
  protected void sendHeaders() throws IOException {
    ...
    // System.out.prntln("Http Status Header = \"" + this.getProtocol() + " " + status + " " + message + "\"");
...
*/
  ...
  public void sendStaticResource() throws IOException {
    ...
    try {
       ...
       * 下面注释掉的是书上的原版代码，图片无法正常显示
       * 问题是，HTTP/1.1版开始传输数据必须要计算总长度，并且在"content-length"参数里告诉浏览器
       * 书上的代码默认的是HTTP/0.9所以不需要计算长度，但现在市面的浏览器已经不支持HTTP/0.9
       * 后面补充的代码计算回应消息的总长度以后，再传输就一切正常
       */
      /*
...
//    if (included)
  ...
  public void flushBuffer() throws IOException {
    ...
    if (bufferCount > 0) {
      try {
        System.out.println("Flush: " + new String(buffer));
